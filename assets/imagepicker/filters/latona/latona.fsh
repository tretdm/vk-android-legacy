 precision lowp float;
 
 varying highp vec2 textureCoordinate;
 
 uniform sampler2D inputImageTexture;
 uniform sampler2D inputImageTexture2; //Map;
 uniform sampler2D inputImageTexture3; //SoftLight
 uniform sampler2D inputImageTexture4; //Curves
 
 void main()
 {
     
     vec3 texel = texture2D(inputImageTexture, textureCoordinate).rgb;
     
     vec2 tc = (2.0 * textureCoordinate) - 1.0;
     float d = dot(tc, tc);
     vec2 lookup = vec2(d, texel.r);
     texel.r = texture2D(inputImageTexture2, lookup).r;
     lookup.y = texel.g;
     texel.g = texture2D(inputImageTexture2, lookup).g;
     lookup.y = texel.b;
     texel.b	= texture2D(inputImageTexture2, lookup).b;
     
     vec3 rgbPrime = vec3(0.1019, 0.0, 0.0);
     float m = dot(vec3(.3, .59, .11), texel.rgb) - 0.03058;
     texel = mix(texel, rgbPrime + m, 0.32);
     
//     texel.r = texture2D(inputImageTexture3, vec2(texel.r, .16666)).r;
//     texel.g = texture2D(inputImageTexture3, vec2(texel.g, .5)).g;
//     texel.b = texture2D(inputImageTexture3, vec2(texel.b, .83333)).b;
     
     texel.r = texture2D(inputImageTexture4, vec2(texel.r, .16666)).r;
     texel.g = texture2D(inputImageTexture4, vec2(texel.g, .5)).g;
     texel.b = texture2D(inputImageTexture4, vec2(texel.b, .83333)).b;
     
     
     gl_FragColor = vec4(texel, 1.0);
 }
